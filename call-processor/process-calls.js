...
// Constants
const API_BASE_URL = process.env.API_BASE_URL || 'https://whisper-training-prep.tess-dev.de/api';
const TRANSCRIPTION_API_URL = process.env.TRANSCRIPTION_API_URL || 'http://localhost:8000/transcribe';
...
async function cutAudio(inputPath, startTime, endTime, outputPath) {
    try {
        ...
        if (endTime > audioDuration) {
            log(`Warning: End time ${endTime} exceeds audio duration ${audioDuration}, setting to duration`, 'warn');
            endTime = audioDuration;
        }
        if (startTime >= endTime) {
            throw new Error(`Invalid segment times: start (${startTime}) must be less than end (${endTime})`);
        }
        ...
        const { stdout: outputDurationStr } = await execAsync(`ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${outputPath}"`);
        ...
        if (durationDiff > 0.1) { // Allow 100ms tolerance
            log(`Warning: Output duration (${outputDuration.toFixed(2)}s) differs from expected (${expectedDuration.toFixed(2)}s) by ${durationDiff.toFixed(2)}s`, 'warn');
        }
        ...
        try {
            ...
            log(`Error cleaning up failed output file: ${cleanupError.message}`, 'warn');
        }
        throw error;
    }
}

async function processSegments(audioPath, segments, outputDir, callId, isLocal = true) {
    const segmentFiles = [];
    let currentSegments = [];
    let segmentIndex = 0;
    ...
    try {
        log(`Starting ${segmentType} segment processing for ${path.basename(audioPath)} (${segments.length} segments)`);
        ...
        for (const segment of segments) {
            if (typeof segment.start !== 'number' || typeof segment.end !== 'number' || 
                segment.start >= segment.end || segment.start < 0) {
                throw new Error(`Invalid segment times: start=${segment.start}, end=${segment.end}`);
            }
        }
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            const currentGroupDuration = currentSegments[currentSegments.length - 1].end - currentSegments[0].start;
            
            // Create new segment if:
            // 1. This is the last segment, or
            // 2. Adding the next segment would make the current group too long
            if (isLastSegment || (nextSegment && (nextSegment.end - currentSegments[0].start) > 30)) {
                // Use padding only for audio cutting, not for segment timings
                const audioCutStart = Math.max(0, currentSegments[0].start - PADDING);
                const audioCutEnd = currentSegments[currentSegments.length - 1].end + PADDING;
                
                log(`Processing ${segmentType} segment group ${segmentIndex + 1}/${Math.ceil(segments.length / 30)} (${audioCutStart.toFixed(2)}s to ${audioCutEnd.toFixed(2)}s, duration: ${(audioCutEnd - audioCutStart).toFixed(2)}s)`);
                log(`Group contains ${currentSegments.length} segments`);
                
                // Create segment file with call ID and segment type in the name
                const outputPath = path.join(outputDir, `segment_${segmentIndex}_${segmentType}_call_${callId}.webm`);
                log(`Cutting audio segment to ${outputPath}`);
                
                try {
                    await cutAudio(audioPath, audioCutStart, audioCutEnd, outputPath);
                } catch (error) {
                    log(`Error cutting ${segmentType} segment group ${segmentIndex + 1}: ${error.message}`, 'error');
                    throw error;
                }
                
                // Upload to S3
                const s3Key = `segments/${path.basename(outputPath)}`;
                log(`Uploading ${segmentType} segment group ${segmentIndex + 1} to S3: ${s3Key}`);
                ...
                try {
                    ...
                    segmentFiles.push({
                        ...
                        originalSegments: currentSegments.map(seg => ({
                            ...
                            text: seg.text,
                            speaker: seg.speaker,
                            words: seg.words ? seg.words.map(word => ({
                                word: word.word,
                                start: word.start,
                                end: word.end,
                                score: word.score
                    ...
                    log(`${segmentType} segment group ${segmentIndex + 1} completed and uploaded successfully`);
                } catch (error) {
                    log(`Error uploading ${segmentType} segment group ${segmentIndex + 1} to S3: ${error.message}`, 'error');
                    throw error;
                }
        ...
        log(`Completed processing all ${segmentType} segments for ${path.basename(audioPath)} (${segmentFiles.length} segment groups created)`);
        return segmentFiles;
    } catch (error) {
        log(`Error processing ${segmentType} segments for ${audioPath}: ${error.message}`, 'error');
        throw error;
    }
}
...
async function processCall(call) {
    ...
    try {
        ...
        log(`Call details: Duration=${call.duration}s, Type=${call.type}, Number=${call.number}`);
        ...
        try {
            ...
            if (localStats.size === 0 || remoteStats.size === 0) {
                throw new Error(`Audio file is empty: ${localStats.size === 0 ? 'local' : 'remote'}`);
            }
            
            log(`Local audio size: ${(localStats.size / 1024 / 1024).toFixed(2)} MB`);
            log(`Remote audio size: ${(remoteStats.size / 1024 / 1024).toFixed(2)} MB`);
            ...
            if (!transcriptionResponse.ok) {
                ...
                log(`Transcription API Response Status: ${transcriptionResponse.status}`, 'error');
                log(`Transcription API Response Headers: ${JSON.stringify(Object.fromEntries(transcriptionResponse.headers.entries()))}`, 'error');
            ...
            if (!transcriptionData.transcript_local || !transcriptionData.transcript_remote || !transcriptionData.transcript_combined) {
                throw new Error('Missing required transcription fields');
            }
            
            // Validate segment structure
            if (!Array.isArray(transcriptionData.transcript_local.segments) || 
        ...
        if (transcriptionData.transcript_local.segments.length > 0) {
            ...
            cutAudioLocal = await processSegments(localAudioPath, transcriptionData.transcript_local.segments, tempDir, call.sql_id, true);
        } else {
        ...
        if (transcriptionData.transcript_remote.segments.length > 0) {
            ...
            cutAudioRemote = await processSegments(remoteAudioPath, transcriptionData.transcript_remote.segments, tempDir, call.sql_id, false);
        } else {
        ...
        if (!updateResponse.ok) {
            ...
            throw new Error(`Failed to update call: ${updateResponse.status} ${updateResponse.statusText}\nURL: ${updateUrl}\nResponse: ${errorText}`);
        }
        ...
        log(`Successfully processed call ${call._id} in ${processingTime}s`, 'success');
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (response.status === 429) { // Too Many Requests
                ...
                log(`Rate limited. Waiting ${waitTime/1000}s before retry ${i + 1}/${retries}...`, 'warn');
            ...
            log(`Request failed. Waiting ${waitTime/1000}s before retry ${i + 1}/${retries}...`, 'warn');
            await delay(waitTime);
        }
    }
}
...
async function processPage(page, limit = BATCH_SIZE) {
    try {
        ...
        const response = await fetchWithRetry(`${API_BASE_URL}/calls?page=${page}&limit=${limit}`);
        ...
        for (const call of data.data) {
            if (call.recordingLocal && call.recordingRemote && !call.transcribed) {
                ...
                log(`Skipping call ${call._id} - ${!call.recordingLocal ? 'missing local recording' : 
                    !call.recordingRemote ? 'missing remote recording' : 'already transcribed'}`, 'warn');
            }
        }
        ...
        try {
            ...
            log(`Warning: Failed to clean up temp directory: ${cleanupError.message}`, 'warn');
        }
        ...
        try {
            ...
            log(`Warning: Failed to clean up temp directory after error: ${cleanupError.message}`, 'warn');
        }
        return null;
    }
}
...
async function main() {
    ...
    try {
        ...
        const initialResponse = await fetchWithRetry(`${API_BASE_URL}/calls?page=1&limit=${BATCH_SIZE}`);
        ...
        while (hasMorePages) {
            ...
            log(`Progress: Page ${currentPage}/${pagination.totalPages} (${processedCalls}/${totalCalls} calls, ${progress}%)`);
        ...
        log(`Finished processing all pages. Total time: ${totalTime}s, Total calls processed: ${processedCalls}`, 'success');
        ...
        try {
            ...
            log(`Warning: Failed to clean up temp directory in final cleanup: ${cleanupError.message}`, 'warn');
        }
        ...
        try {
            ...
            log(`Warning: Failed to clean up temp directory after main error: ${cleanupError.message}`, 'warn');
        }
        main()
    }
}
